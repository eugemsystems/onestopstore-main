...
const nextConfig = {
  ...
  // The sitemap index at /sitemap.xml (src/app/sitemap.xml/route.js) points to
  // individual /v{N}-sitemap-*.xml files generated by the Laravel backend
  // (GenerateSitemaps.php) — proxy those straight through so they resolve
  // same-origin, exactly as Google expects for a sitemap index's children.
  async rewrites() {
    const apiBaseUrl =
    ...
    return [
      {
        source: "/:file(v\\d+-sitemap(?:-[^/]+)?\\.xml)",
        destination: `${apiBaseUrl}/sitemaps/:file`,
      },
    ];
  },
  ...
  images: {
    ...
    remotePatterns: [
      ...
      { protocol: "https", hostname: "media.onestopstore.local" },
      { protocol: "http", hostname: "media.onestopstore.local" },
      { protocol: "https", hostname: "media.onestopstore.co.zw" },
      { protocol: "http", hostname: "media.onestopstore.co.zw" },
      { protocol: "https", hostname: "api.onestopstore.local" },
      { protocol: "http", hostname: "api.onestopstore.local" },
